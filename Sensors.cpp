...
// ****************
void GYRO_Common() {
  static int32_t previousGyroADC[3] = {0,0,0};
  static int32_t g[3];
  uint8_t axis=0;
...
void Gyro_init() {
  ...
  i2c_writeReg(MPU6050_ADDRESS, 0x1B, 0<<3);             //GYRO_CONFIG   -- FS_SEL = 0: scale set to 250 deg/sec
...
void Gyro_getADC () {
  ...
  GYRO_ORIENTATION( ((rawADC[0]<<8) | rawADC[1]) , // range: +/- 32768; +/- 250 deg/sec
                    ((rawADC[2]<<8) | rawADC[3]) ,
                    ((rawADC[4]<<8) | rawADC[5]) );
  GYRO_Common();
}
